...
#define Rolling_Compute() \
    ((count >= min_count) ? monotonic_queue_method(front_value, SourceType)(queue) : NPY_NAN)
...
PyObject* rolling_max(PyObject* self, PyObject* args, PyObject* kwargs) {
    ...
    if (dtype == NPY_FLOAT64) {
        rolling_max_npy_float64(arr, max, window, min_count, axis);
    } else if (dtype == NPY_FLOAT32) {
        rolling_max_npy_float32(arr, max, window, min_count, axis);
    } else if (dtype == NPY_INT64) {
        rolling_max_npy_int64(arr, max, window, min_count, axis);
    } else if (dtype == NPY_INT32) {
        rolling_max_npy_int32(arr, max, window, min_count, axis);
    } else {
